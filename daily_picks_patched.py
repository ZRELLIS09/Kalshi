...
def compute_probability(features):
    """Convert features to home cover probability via weighted logistic."""
    logit = 0.0
...
def predict_game(home_name, away_name, spread, team_lookup):
    ...
    expected_margin = features.get("rating_diff", 0.0) + HOME_COURT_ADVANTAGE
    ...
    if prob_home > 0.5:
        pick, pick_side, confidence = home_norm, "HOME", prob_home
        pick_spread = f"{home_norm} {spread:+.1f}" if spread is not None else home_norm
    else:
        pick, pick_side, confidence = away_norm, "AWAY", 1.0 - prob_home
    ...
    if conf_pct >= 90:    conf_tier = f"ðŸ”’ {conf_pct:.1f}%"
    elif conf_pct >= 80:  conf_tier = f"ðŸ“Š {conf_pct:.1f}%"
    elif conf_pct >= 65:  conf_tier = f"ðŸ“‰ {conf_pct:.1f}%"
    else:                 conf_tier = f"âšª {conf_pct:.1f}%"
...
def format_pick_line(row):
    """Format single pick with confidence %."""
